...
- Stage 1 (a.k.a. initrd) is now based on systemd by default, and the old scripted implementation is deprecated and scheduled for removal in 26.11. If you run into issues migrating, you can [get help from the community](https://nixos.org/community/) or report an issue on GitHub.
  ...
  - `nixos-rebuild` and `nixos-install` now support system.nix in addition to configuration.nix and flae.nix. system.nix is designed to allow greater flexibility in the entry-point to which a NixOS configuration modules is passed (i.e. other than `nixpkgs/nixos/default.nix`). In addition, system.nix brings non-flakes workflows closer to parity with flakes workflows by providing a mechanism to define multiple NixOS systems within a single file.

  system.nix must evaluate to a NixOS system derivation or an attribute set of such derivations, in which case the attribute to build has to be selected with the `--attr` option of `nixos-rebuild` or `nixos-install`.
  For example,
  ```nix
  # system.nix
  let
